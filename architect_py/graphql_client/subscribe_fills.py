# Generated by ariadne-codegen
# Source: queries.graphql

from typing import Any, Optional

from pydantic import Field

from .base_model import BaseModel
from .enums import FillKind
from .fragments import MarketFields


class SubscribeFills(BaseModel):
    fills: "SubscribeFillsFills"


class SubscribeFillsFills(BaseModel):
    dir: Any
    fill_id: Any = Field(alias="fillId")
    kind: FillKind
    market_id: Any = Field(alias="marketId")
    order_id: Optional[Any] = Field(alias="orderId")
    price: Any
    quantity: Any
    recv_time: Optional[Any] = Field(alias="recvTime")
    trade_time: Any = Field(alias="tradeTime")
    market: "SubscribeFillsFillsMarket"


class SubscribeFillsFillsMarket(MarketFields):
    pass


SubscribeFills.model_rebuild()
SubscribeFillsFills.model_rebuild()
